...
export const NavbarItemSubMenu: React.FC<props> = ({
  navItem,
  subMenuActive,
  setSubmenuActive,
}) => {
  return (
    <div
      className={combineClass(
        ...
        {
          "pointer-events-auto translate-x-0 opacity-100 lg:translate-y-0":
            subMenuActive,
        },
      )}
    >
      <div
        className={combineClass(
          "container mx-auto translate-x-0 transform opacity-0 transition-all duration-1000 ease-out",
          {
      ...
      >
        ...
        >
          {navItem.children?.map(
            (navbarItemSubMenuChild: any, indexChildNav: number) => (
              <div key={indexChildNav}>
                <NavbarItemSubMenuChild
                  key={indexChildNav}
                  navbarItemSubMenuChild={navbarItemSubMenuChild}
                />
              </div>
            ),
          )}
