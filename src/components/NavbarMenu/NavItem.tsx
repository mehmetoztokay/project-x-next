...
export const NavItem = ({navItem}: any) => {
  ...
  return (
    <div ref={navItemRef} className="flex items-center">
      {navItem.hasChildren ? (
        <button
          onClick={() => setIsOpenNavItem(!isOpenNavItem)}
          className={combineClass("hover:bg-gray-200 px-3 py-1 rounded-md transition-all ease-in duration-400 flex gap-1 items-center", {"text-blue-500": isOpenNavItem})}
        >
          {navItem.title}
          <svg
            className={combineClass("fill-gray-600", {"transform rotate-180 fill-blue-500": isOpenNavItem})}
            width="10"
            height="10"
            viewBox="0 0 16 16"
            fill="none"
      ...
      {navItem.hasChildren && isOpenNavItem && (
        <div className="absolute top-0 left-0 backdrop-blur-md bg-gray-200/30 inset-0 -z-10 pt-20 pb-16 h-fit">
          <div className="container relative mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 flex-wrap gap-x-10 gap-y-14 border-t-2 border-t-gray-200 pt-8">
            <button onClick={() => setIsOpenNavItem(false)} className="absolute right-[10px] top-4 bg-gray-200 hover:bg-gray-300 transition-all ease-in rounded-full p-1">
            ...
            {navItem.children?.map((childNav: any, indexChildNav: number) => (
              <div className={combineClass("pl-3", navItem.children.length > 1 && "border-l-2 border-l-gray-200")} key={indexChildNav}>
