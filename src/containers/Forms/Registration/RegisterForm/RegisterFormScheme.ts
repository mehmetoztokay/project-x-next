...
export const getSingleRegisterFormScheme = (tForm: any) => {
  const scheme = Yup.object().shape({
    ...
    // PhoneCode
    phoneCode: Yup.string().required(tForm("isRequired")).matches(regexPhoneCode, tForm("invalidPhoneCode")),
    // Consent Marketing
...
export const getStep1FormScheme = (tForm: any) => {
  const scheme = Yup.object().shape({
    // Email
    email: Yup.string()
      .required(tForm("isRequired"))
...
export const getStep2FormScheme = (tForm: any) => {
  const scheme = Yup.object().shape({
    siteId: Yup.number().required(tForm("isRequired")),
    email: Yup.string().required(tForm("isRequired")).email(tForm("invalidEmail")).matches(regexMail, tForm("invalidEmail")),
    scaPassword: Yup.string()
      .required(tForm("isRequired"))
      .matches(regexNoSpecialChars, tForm("noSpecialChars"))
      .min(8, tForm("minChars", { number: 8 }))
