...
export const CheckboxField = ({ name, label, children, className, hideErrorMessage = false }: CheckboxProps) => {
  ...
  return (
    <>
      {" "}
      <label className={combineClass("relative flex cursor-pointer select-none ps-6 text-lg", className, {})}>
        <input type="checkbox" {...field} className="peer absolute h-0 w-0 opacity-0" />
        ...
        <div className="mt-[3px] inline-block text-justify text-xs text-gray-500 lg:mt-[1px] lg:text-sm">{label ? label : children}</div>
      </label>
      {!hideErrorMessage && meta.touched && meta.error ? <p className="-mt-2 ps-6 text-xs text-red-500">{meta.error}</p> : null}
    </>
  );
};
